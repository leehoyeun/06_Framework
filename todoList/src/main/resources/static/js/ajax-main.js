...
method : "PUT",
headers : {"Content-Type" : "application/json"},
body : JSON.stringify(obj) // JS 객체를 JSON 형태로 변환하여 전달
})
.then(resp => resp.text())
.then(result => {
  ...
  if(result > 0) { // 성공
    ...
    //update 된 DB 데이터를 다시 조회 해서 화면에 출력
    // ->서버 부하가 큼

    //selelect();
    //상세 조회에서 Y/N만 바꾸기
    popupComplete.innerText = complete; // Y/N 변경


    //getCompleteCount(); // 완료된 할 일 개수 다시 조회
    //완료된 Todo 개수 +1 -1

    const count = Number(completeCount.innerText);
    if(complete === 'Y') completeCount.innerText = count + 1; // 완료된 개수 +1
    else completeCount.innerText = count - 1; // 완료된 개수 -1
...
updateView.addEventListener("click", () => {
  ...
  popupLayer.classList.add("popup-hidden"); // 상세조회 레이어 숨기기
  ...
  // textarea 에서는 \n 으로 바꿔줘야 실제 줄바"꿈으로 인식되어 출력한다!

  //수정 레이어 -> 수정버튼에 data-todo-no 속성 추가
...
updateBtn.addEventListener("click", e => {
  ...
  .then(result => {
    ...
    if(result >0){
      ...
      //selectTodo();
      //-> 성능개선
      popupTodoTitle.innerText = updateTitle.value; // 제목
      ...
      updateTitle.value = "";
      updateContent.value = "";
      updateBtn.remobeAttribute("data-todo-no"); // data-todo-no 속성 제거
