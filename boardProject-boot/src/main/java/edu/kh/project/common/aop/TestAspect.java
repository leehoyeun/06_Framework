 ...
 * 
 * 
 *   @Cmponent -> 클래스 단순히 Bean 등록
 *   @Bean -> 개발자가 수동으로 만든 객체를 Bean 등록
 *   두개가 다르니 잘찾아보고 이해해보자
 * */
...
public class TestAspect {
	...
	// Advice : 끼워 넣을 코드(메서드)
	//Pointcut : 실제로 Advice 를 적용한 JoinPoint (지점)
	
	// <Pointcut 작성법>
	// execution([접근제한자] 리턴타입 클래스명 메서드면([파라미터]) )
	// 클래스명은 패키지명부터 모두 작성!
	
	// execution(* edu.kh.project..*Controller*.*(..))
	
	// execution() : 머서드 실행 지점을 가리키는 키워드
	// * : 모든 리턴 타입을 타입을 나타냄
	...
	public void controllerEnd(JoinPoint jp) {
		...
		String methodNmae = jp.getSignature().getName(); // ex)mainPage(),loing..
		
		log.info("--------{}.{} 수행완료-----------",className,methodNmae);
	}
