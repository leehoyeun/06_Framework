...
if(profileForm!=null){
    ...
    const MAX_SIZE = 1024 * 1024 * 5;  // 최대 파일 크기 설정 (5MB) 5MB를 Byte로 표시함

    //절대경로 방식으로 기본 이미지 URL 설정
    const defalutImageUrl = `${window.location.origin}/images/user.png`;
    // http://localhost/images/user.png

    let statusCheck = -1; // -1 : 초기상태 , 0:이미지 삭제, 1:새이미지 선택

    //img 태그에 자겅하는 값 (src) = 미리보기 이미지를 띄울 URL 주소
    let previousImage = profileImg.src;  // 이전 이미지 URL 기록 (초기 상태의 이미지URL 저장)

    // input (type=file) 태그가 작성할 값= 서버에 실제로 제출되는 File 객체
    let previousFile = null; //이전에 선택된 File 객체를 저장할 변수
    ...
    imageInput.addEventListener("change",()=>{
        ...
        if(file){ //파일이 선택된 경우
            ...
            if(file.size <= MAX_SIZE){//현재 선택한 파일의 크기가 허용범위 이내인 경우(정상인 경우)
                ...
                profileImg.src= newImageUrl;//미리보기 이미지 설정
                                    ...
                                    //경로를 들어가면 오류가나지만 url값을 입력하면 쓸수 있음
                
                previousImage = newImageUrl; //현재 선택된 이미지를 이전 이미지로 저장(다음에 바뀔일에 대비)
                previousFile = file; // 현재 선택된 파일객체를 이전 파일로 저장(다음에 잘못 바뀔것에 대한 대비)
        ...
        }else{//파일이 선택되지 않은경우(==취소를 누른경우)

            // 이전에 미리보기 이미지로 복원하기(img)
            profileImg.src = previousImage;
            
            // 이전 선택한 파일로 복원하기(input)
            if(previousFile){//이전 파일이 존재한다면
                ...
                // --> input 태그에 files 속성은 FileList만 저장 가능한 형태이기 때문에
    ...
    deleteImage.addEventListener("click",()=>{
        ...
        if(profileImg.src !== defalutImageUrl){
            imageInput.value=""; //input 태그 파일값 초기화
            profileImg.src = defalutImageUrl;//현재 미리보기를 기본 이미지로 변경
            statusCheck = 0; //삭제 상태 기록
            previousImage = defalutImageUrl; // 이전 이미지 기록하는 변수에 기본이미지로 변경
            previousFile = null; // 이전 파일 기록하는곳에 null로 초기화
            
        }else{// 기본 이미지 상태에서 삭제 버튼 클릭 시 상태를 변경하지 않음
            statusCheck = -1; // 변경 사항 없음
        }
