...
// - trueL : 해당 항목은 유효한 형식으로 작성됨

const checkobj = {
    "memberEmail": false, //이메일 유효성 검사 여부
    "memberPw": false, //비밀번호 유효성 검사 여부
    "memberPwconfirm" : false, //비밀번호 확인 유효성 검사 여부
    "memberNickname": false, //닉네임 유효성 검사 여부
    "memberTel" : false, //전화번호 유효성 검사 여부
    "authKey" : false //인증번호 유효성 검사 여부
};
...
//인증번호 관련 메시지 확인후 결과 메시지 출력 span 요소
const authKeyMessgae = document.querySelector("#authKeyMessage");
...
sendAuthKeyBtn.addEventListener("click",()=>{
  ...
  // 화면에서는 타이머 시작하기
  authKeyMessgae.innerText =initTime;
  authKeyMessgae.classList.remove("confirm","error");//검정 글씨색
  ...
  //이증 가능 시간 출력 (1초 마다 동작)
  authTimer = setInterval(()=>{

    authKeyMessgae.innerText =`${addZero(min)}:${addZero(sec)}`;

    // 0분 0초인 경우 ("00:00 출력후")
    if(min == 0 && sec ==0){
      checkobj.authKey= false;
      clearInterval(authTimer);
      authKeyMessgae.classList.add('error');
      authKeyMessgae.classList.remove('confirm');
      return;
    }
...
// 매개변수로 전달받은 숫자가 10미만인 경우(한자리) 앞에 0붙여서 반환
