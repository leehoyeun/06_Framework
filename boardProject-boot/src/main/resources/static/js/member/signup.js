...
checkAuthKeyBtn.addEventListener("click",()=>{
  ...
  if(authKey.value.length < 6|| authKey.value.length >= 7){ //인증번호가 제대로 입력 안된 경우 (길이가 6미만 또는 7이상일때)
  ...
  .then(result =>{
    ...
    checkobj.authKey= true; //인증번호 검수 여부 true 변경
  });
...
memberNickname.addEventListener("input",e=>{
  ...
  if(!regExp.test(inputNickname)){//유효하지 않은경우
...
memberTel.addEventListener("input",e=>{
  ...
  telMessage.innerText="유효하지 전화번호 형식입니다.";
...
signUpForm.addEventListener("submit",e=>{
  ...
  for(let key in checkobj){ // checkobj 요소의 key값을 순서대로 꺼내옴
    ...
    switch(key){
      ...
      case "authKey" : str ="이메일이 인증되지 않습니다,";break;
